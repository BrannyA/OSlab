...
desc_t Tx_desc[PNUM];  // Send
desc_t Rx_desc[PNUM];  // Receive
uint32_t Tx_buffer[PNUM][PSIZE];  // Send
uint32_t Rx_buffer[PNUM][PSIZE];  // Receive
...
static void mac_recv_desc_init(mac_t *mac)
{
    ...
    test_mac_recv_glb = (uint32_t)&Rx_desc[0];
}
...
static uint32_t printf_recv_buffer(uint32_t recv_buffer)
{
    ...
    int i,value;
    for(i=0;i<64;i++){
        value = *(uint32_t *)(recv_buffer+4*i);
        printf("%x ",value);
    }
    return 1;
}
...
void mac_recv_task()
{
    ...
    for (i = 0; i < PNUM; i++)
    {
        ...
        sys_move_cursor(1, print_location);
        printf("\n%d recv buffer,r_desc( 0x%x) =0x%x:          flag=%d\n", i, Recv_desc, *(Recv_desc), recv_flag[i]);
