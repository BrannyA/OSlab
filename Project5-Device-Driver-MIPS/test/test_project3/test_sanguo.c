    ...
    for (i = 0;; i++) {
        ...
        switch (i % 2) {
            case 0:
                ...
                sys_move_cursor(0, 5);
                printf("[CaoCao]biu biu biu ~~~~~~ AAAAAAAA SunQuan is dead.");
                sys_kill(sunquan);
                mbox_recv(subSunQuan, &sunquan, sizeof(pid_t));

                sys_move_cursor(0, 6);
                printf("[CaoCao](%d):  SunQuan(%d) lives!                 ", myPid, sunquan);
                break;
            case 1:
                sys_move_cursor(0, 4);
                ...
                sys_move_cursor(0, 5);
                printf("[CaoCao]biu biu biu ~~~~~~ AAAAAAAA Liubei is dead .");
                sys_kill(liubei);
                mbox_recv(subLiuBei, &liubei, sizeof(pid_t));

                sys_move_cursor(0, 6);
                printf("[CaoCao](%d): LiuBei(%d) is alive again! ", myPid, liubei);
                break;
        }
